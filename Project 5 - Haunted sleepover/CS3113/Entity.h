...
enum EntityType { PLAYER, BLOCK, PLATFORM, NPC, NONE, DOOR, ITEM };
enum AIType { WANDERER, FOLLOWER, FLYER };
enum AIState { WALKING, IDLE, FOLLOWING, ATTACKING};
...
private:
    ...
    Vector2 mPatrolStart = { 0, 0 };
    Vector2 mPatrolEnd = { 0, 0 };
    bool mPatrolInitialized = false;
    bool mWalkingLeft = true;
    ...
    Texture2D mJumpTexture;
    Texture2D mAttackTexture;
    Texture2D mSprintTexture;
    ...
    bool mAnimationJustChanged = false;
    ...
    void updateWandererAnimationState();
    void updateFollowerAnimationState();
    void updateFlyerAnimationState();
...
public:
    ...
    void makeInvincible(float duration = 1.0f) {
        mIsInvincible = true;
    ...
    void loadAnimationTextures(const char* idlePath, const char* runPath,
