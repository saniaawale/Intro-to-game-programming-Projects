    ...
    Entity* collidableEntities, int collisionCheckCount)
{
    ...
    if (mEntityType == PLAYER) {
        ...
        updateEnemyAnimationState();  // Enemy uses this
    }
...
void Entity::loadAnimationTextures(const char* idlePath, const char* runPath,
    const char* jumpPath, const char* attackPath)
{
...
void Entity::updateEnemyAnimationState()
{
    if (!mTexturesLoaded || mEntityType != NPC) return;

    switch (mAIType)
    {
    case WANDERER:
        updateWandererAnimationState();
        break;
    case FOLLOWER:
        updateFollowerAnimationState();
        break;
    case FLYER:
        updateFlyerAnimationState();
        break;
    default:
        break;
    }
...
void Entity::updateFollowerAnimationState()
{
    if (!mTexturesLoaded || mEntityType != NPC || mAIType != FOLLOWER) return;

    Texture2D previous = mTexture;

    bool isMoving = (fabs(mMovement.x) > 0.01f || fabs(mMovement.y) > 0.01f);
    if (isMoving) {
        mTexture = mRunTexture;  // Walk animation
        mSpriteSheetDimensions = { 1, 4 };
    }
    else {
        mTexture = mIdleTexture;  // Idle animation when not moving
        mSpriteSheetDimensions = { 1, 4 };
    }
    if (mTexture.id != previous.id || mAnimationIndices.empty()) {
        mAnimationIndices.clear();
        int frameCount = (int)mSpriteSheetDimensions.y;
...
void Entity::updateWandererAnimationState()
{
    if (!mTexturesLoaded || mEntityType != NPC || mAIType != WANDERER) return;
    ...
    if (isMoving) {
        mTexture = mRunTexture;
        mSpriteSheetDimensions = { 1, 8 };  //Run animation: 8 frames
    }
    bool textureChanged = (mTexture.id != previous.id);
    if (textureChanged || mAnimationIndices.empty()) {
        // Rebuild animation indices
        mAnimationIndices.clear();
        int frameCount = (int)mSpriteSheetDimensions.y;
...
void Entity::updateFlyerAnimationState()
{
    if (!mTexturesLoaded || mEntityType != NPC || mAIType != FLYER) return;
    ...
    mTexture = mRunTexture;  // Flying animation
    mSpriteSheetDimensions = { 1, 8 };  // Adjust based on your sprite sheet
